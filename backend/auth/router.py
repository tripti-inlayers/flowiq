...
#   1. Line 29: replaced `from ..database import supabase`
#              with `from db.connection import supabase`
#   2. db_get_user_by_email / db_get_user_by_id: wrapped in try/except
#      because .single() raises PostgrestAPIError when no row found
#      (previously would crash instead of returning None)
...
# ── DB helpers ────────────────────────────────────────────
# ✅ Fixed: .single() raises an exception when 0 rows found.
#    We catch it and return None so callers get a clean signal.
